(* E20 - For noForth C&V 200202: bit output, timer interrupt with machine code, 
...
routine TIMER-INT   ( -- )
    ...
    =? if,                  \ Zero?
        ...
        41 # 21 & bix       \ P1OUT  Toggle both LEDs
    then,
    reti
end-code
...
: SMCLK-ON      ( -- )
    int-off  41 22 *bis     \ P1DIR    Set pins with LED1,2 to output
    01 21 *bis 40 21 *bic   \ P1OUT    Set red led on, green off
    264 160 !               \ TA0CTL   Set timer mode to SMCLK/2
