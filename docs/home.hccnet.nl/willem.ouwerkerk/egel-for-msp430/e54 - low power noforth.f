(* E54 - For noForth C&V2553 lp.0, Low Power Mode, RS232 interrupt
  ...
  0021 - P1OUT          - Port-1 Ouput reg.
  0001 - IE2    Bit-0,1 - Interrupt Enable reg. 2
  0003 - IFG2   Bit-0,1 - Interrupt Flag reg. 2
...
code EMIT}      ( c -- )
    ...
    #2 001 & .b bis         \ IE2  RS232 TX interrupt on
    ...
    then,
    tos 067 & .b mov        \ UCA0TXBUF  Send char
    sp )+ tos mov           \ Pop tos
    NEXT
end-code
...
code KEY}       ( -- c )
    tos sp -) mov           \ Push TOS
    #1 003 & .b bit         \ IFG2  Test RX flag
    =? if,                  \ Not set ?
        #8 sr bic           \ interrupt off ^^^ Important!
        #1 001 & .b bis     \ IE2   RX interrupt on
        #8 adr ok & bit     \ Which power mode?
        <>? if,
            98 # sr bis     \ Go from AM to LPM2
        else,
...
routine WAKE-RX    ( -- )
    #1 0001 & .b bic        \ IE2  RS232 RX int. off
    F0 # rp ) bic           \ LPM off
    reti
end-code
...
routine WAKE-TX    ( -- )
    #2 0001 & .b bic        \ IE2  RS232 TX int. off
    F0 # rp ) bic           \ LPM off
   reti
end-code
...
: (MS)          ( u x -- )
    ...
    01 000 *bis  lpm0       \ IE1     Go to sleep with interrupt on
    5A91 120 !  01 000 *bic \ WDTCTL, IE1  Stop watchdog & interrupt off
    ;

\ This variant should always be correct
: MS1           ( u -- )
    012A c@  ( FCTL2  Use  flash timing as reference )
...
: SLEEP-ON      ( -- )  \ Set LPM RS232 variant routines
    int-off
    F7 022 *bis         \ P1DIR  Only P1.3 is input
    41 021 *bic         \ P1OUT  Leds off
    10 029 *bic         \ P2OUT  Set P2.4 (mosfet) off
