...
ALL  ( dc - )     - Set all leds to color from the stack
...
BOUNCE ( u - )    - Special effect 2 bouncing dot
...
value END?
: UP/DOWN   ( -- )
    wait  rc5key >r             \ +n        - x1 on top
    r@ 10 = if  5 +    then     \ +n (vol+) - Increase 5
    r@ 11 = if  5 -    then     \ +n (vol-) - Decrease 5
    r@ 20 = if  1E +   then     \ +n (P+)   - Increase 30
    r@ 21 = if  1E -   then     \ +n (P-)   - Decrease 30
    r@ 12 = if  cswap  then     \ +n (MENU) - swap colors
    r> 17 = if  true to end?  then  \ Ready
    00 max >time ;              \ +n            - Not below zero!

: <WAIT>    ( -- )      rckey? if  up/down  else  wait ms  then ;
