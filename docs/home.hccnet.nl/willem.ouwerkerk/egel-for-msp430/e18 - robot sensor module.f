(* E18 - For noForth C&V2553 lp.0, Bit input & output with MSP430G2553
...
: SETUP-SENSORS ( -- )  \ Initialise the simple sensor module
    38 022 *bic         \ P1DIR  P1.3, 4 & 5 inputs
    40 022 *bis         \ P1DIR  P1.6 output
    78 027 *bis         \ P1REN  Resistors on P1.3 to P1.5 on
    78 021 *bis ;       \ P1OUT  and pullups, also leds on


: MOTION?       ( -- flag )     08 020 bit* 0= ; \ Flag is true when motion is detected
: TOUCH-LEFT?   ( -- flag )     10 020 bit* 0= ; \ Flag is true when feeler is bend
: TOUCH-RIGHT?  ( -- flag )     20 020 bit* 0= ;
: ?LED          ( flag -- )     if  40 021 *bis  else  40 21 *bic  then ;
