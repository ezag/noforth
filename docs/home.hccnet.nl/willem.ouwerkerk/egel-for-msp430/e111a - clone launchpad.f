(* E111 – For noForth C&V 200202 or later: CLONE noForth to another MSP430G2553
  ...
  to generate and restore a set of new DCO-constants. For more information
  on the built in capacitators read SLAU144J.PDF page 274 and beyond.
  ...
  CATEGORY    : Application, size: ~1340 bytes.
  AUTHOR      : Willem Ouwerkerk, 12 April 2016
  LAST CHANGE : Willem Ouwerkerk, 07 june 2016
  ----------------------------------------------------------------------
...
hex chere  ( Remember CHERE for space calculation )
...
\ The parity bit is not checked in this implemention!
code BSL-KEY    ( -- char )     \ rs232s read char from rs232
    ...
    #8 moon mov                 \ read 8 databits & parity bit
    begin,                      \ Leave stopbit
        wait-bit # call         \ wait bittime
...
\ ) 20 # 21 & bix               \ Trace stopbit moment at P1.5
    tos swpb
    #-1 tos .b bia  next        \ Use low byte only
end-code
...
(* DCO calibration for noForth C&V for MSP430G2553
    ...
    In a word as stored in info-A: |BSCTL1|DCOCTL|
 *)
...
code {I         ( -- )  \ Activate INFO-A Flash write sequence
...
code I}         ( -- )  \ End INFO-A Flash write sequence
    A500 # 0128 & mov
...
( ) begin  1 053 bit* 0= until \ Wait for LFXT1 correct
    ...
    swap  10F8 10FE do  i {i ! i}  -2 +loop ; \ Write to info-A
...
\ DB14 modify-pass      \ Set BSL noForth password C2553 LP 160202
  empty-pass            \ Set BSL for empty MSP430G2553 password
freeze
