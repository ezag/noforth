(* E32U - For noForth C&V2553 lp.0, bitbang I2C on MSP430G2553 using port-1.
  I2C input & output with a PCF8574 & PCF8583 using external pull-ups

  Connect the ì2print from the Forth users group or any other module
...
: !CLOCK         ( x a -- )     \ set x in address addr
    A0 {i2write                 \ send dev. address write
    i2out1  i2out  i2stop} ;    \ send address, databyte & stopcond.

\ Read data 'x' from address 'a' from PCF8583.
: @CLOCK         ( a -- x )     \ Read de contents from addr.
    A0 {i2write  i2out1         \ dev. address & clock address
...
: TIMER         ( -- )          \ Show timer
    ...
    begin
        next-alarm              \ Advance alarm with 10 sec.
...
: CLOCK         ( -- )
     setup-i2c  00 00 !clock  00 08 !clock \ Normal clock, no alarm
     base @ >r  decimal
     begin
         get-sec sec <> if
            get-sec to sec
